...
public class TitleExtractor {
	private static List<String> ignoreSuffixes = Arrays.asList("频道", "站", "网", "报", "公司", ".com", ".cn", "平台", "门户", "博客");
	...
	private String getLongestPart(List<String> parts) {
		...
		for (int i = 0; i < parts.size(); i++) {
			...
			if (colonCnt > 0) ignoreCount += colonCnt - 1;
			double numWords = TextUtility.countNumWords(p);
	...
	private List<String> split(String title) {
		...
		for (int i = 0; i < title.length(); i++) {
			...
			if (quoteCnt == 0 && !lastLetter && splitChars.contains(ch)) {
	...
	public static void main(String[] args) {
		...
		String[] titles = new String[] {
				"《火车》(Helpless)YYeTs_韩剧精灵原创听译 HR-HDTV,RMVB,磁力下载电驴下载,快车下载,中转站下载,简 |YYeTs,人人影视,分享 学习 进步 ——人人影视字幕组,永远为您翻译最快最好的海外影视字幕",
