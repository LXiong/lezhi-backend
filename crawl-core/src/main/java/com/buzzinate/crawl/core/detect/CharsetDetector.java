...
//          working.
        ...
        int maxConf = matches[0].getConfidence();
        for (CharsetMatch cm: matches) {
        	if (cm.getName().equalsIgnoreCase(fDeclaredEncoding) && cm.getConfidence() >= maxConf * 9 / 10) return cm; 
        }
        
        return matches[0];
     }
    
     public List<CharsetMatch> detectTop() {
   //   TODO:  A better implementation would be to copy the detect loop from
//             detectAll(), and cut it short as soon as a match with a high confidence
//             is found.  This is something to be done later, after things are otherwise
//             working.
           ...
           List<CharsetMatch> results = new ArrayList<CharsetMatch>();
           ...
           int maxConf = matches[0].getConfidence();
           for (CharsetMatch cm: matches) {
           	if (cm.getConfidence() >= maxConf * 9 / 10) results.add(cm); 
           }
