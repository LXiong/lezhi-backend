...
public class AtomSeg {
	...
	private ArrayList<Atom> atomSplit() {
		...
		if (str != null && str.length() > 0) {
			...
			for (int i = index; i < ss.length; i++) {
				...
				if (".".equals(s) && (i + 1 < ss.length && (Utility.charType(ss[i + 1]) == Utility.CT_NUM || GFString.isNumeric(ss[i+1]))))
				...
				// 如果是数字、字母、单字节符号，则把相邻的这些做为一个原子。比如：三星SHX-123型号的手机，则其中的SHX-123就是一个原子
					...
					if (nextType != curType || i == ss.length - 1 || !isFirstLetter(sAtom)) {
						if (curType == Utility.CT_SINGLE && GFString.isNumeric(sAtom)) curType = Utility.CT_NUM;
						Atom atom = new Atom();
						atom.setWord(sAtom);
	...
	public static void main(String[] args) {
		String text = "其中的SHX-123就是一个90%原子, 混合动力轿车长轴距版----M35hL";
		List<Atom> atoms = new AtomSeg(text).getAtoms();
