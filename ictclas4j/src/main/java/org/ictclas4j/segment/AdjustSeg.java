	...
	public static ArrayList<SegNode> finalAdjust(ArrayList<SegNode> optSegPath, DictLib dictLib) {
		...
		if (optSegPath != null && optSegPath.size() > 0 && personTagger != null && placeTagger != null) {
			...
			for (int i = 0; i < optSegPath.size(); i++) {
				...
				if (wr.getPos() == POSTag.NUM && i + 2 < optSegPath.size() && optSegPath.get(i + 1).getLen() == 1
						&& optSegPath.get(i + 1).getSrcWord().equals(optSegPath.get(i + 2).getSrcWord())) {
					SegNode wr2 = new SegNode();
					String srcWord = wr.getSrcWord() + optSegPath.get(i + 1).getSrcWord() + optSegPath.get(i + 2).getSrcWord();
					wr2.setSrcWord(srcWord);
					wr2.setWord(srcWord);
				...
				else if (wr.getLen() == 1 && i + 1 < optSegPath.size() && wr.getSrcWord().equals(optSegPath.get(i + 1).getSrcWord())) {
					SegNode wr2 = new SegNode();
					String srcWord = wr.getSrcWord() + optSegPath.get(i + 1).getSrcWord();
					wr2.setSrcWord(srcWord);
					wr2.setWord(srcWord);
				...
				// Rule 4: AAB 洗/洗澡
				else if (wr.getLen() == 1 && i + 1 < optSegPath.size() && (wr.getPos() == POSTag.VERB || wr.getPos() == POSTag.ADJ)
						&& optSegPath.get(i + 1).getLen() == 2 && optSegPath.get(i + 1).getSrcWord().indexOf(wr.getSrcWord()) == 0) {
					SegNode wr2 = new SegNode();
					String srcWord = wr.getWord() + optSegPath.get(i + 1).getSrcWord();
					wr2.setSrcWord(srcWord);
					wr2.setWord(srcWord);
					...
					wr.setPos('u' * 256);
				// AABB,朴朴素素
				else if (wr.getLen() == 1 && i + 2 < optSegPath.size() && optSegPath.get(i + 1).getLen() == 2
						&& optSegPath.get(i + 1).getWord().indexOf(wr.getWord()) == 0
						&& optSegPath.get(i + 1).getWord().indexOf(optSegPath.get(i + 2).getWord()) == 0) {
					SegNode wr2 = new SegNode();
					String srcWord = wr.getWord() + optSegPath.get(i + 1).getWord() + optSegPath.get(i + 2).getWord();
					wr2.setSrcWord(srcWord);
					wr2.setWord(srcWord);
				...
				else if (wr.getPos() == POSTag.NOUN_SPACE && i + 1 < optSegPath.size())// PostFix
				{
					...
					if (placeTagger.isExist(next.getSrcWord(), 4, gbkID)) {
						SegNode wr2 = new SegNode();
						String srcWord = wr.getSrcWord() + next.getSrcWord();
						wr2.setSrcWord(srcWord);
						wr2.setWord(srcWord);
						...
						isBeProcess = true;
						result.add(wr2);
					} else if (optSegPath.get(i + 1).getLen() == 1 && "语文字杯".indexOf(next.getSrcWord()) != -1) {
						SegNode wr2 = new SegNode();
						String srcWord = wr.getSrcWord() + next.getSrcWord();
						wr2.setSrcWord(srcWord);
						wr2.setWord(srcWord);
