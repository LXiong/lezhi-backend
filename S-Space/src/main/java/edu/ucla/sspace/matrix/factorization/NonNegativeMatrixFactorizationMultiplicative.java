...
public class NonNegativeMatrixFactorizationMultiplicative
    ...
    private static final Logger log = Logger.getLogger(NonNegativeMatrixFactorizationMultiplicative.class);
    ...
    public void factorize(SparseMatrix matrix, int numDimensions) {
        ...
        for (int i = 0; i < outerLoop; ++i) {
            ...
            for (int j = 0; j < innerLoop; ++j) {
                ...
                log.debug("Step 3: " + (end-start) + "ms");

                // Update H with the new value.
                H = Hprime;
            }

            log.debug("Updating W matrix");
            // Update the H matrix by holding the W matrix fixed for a few
            // iterations.
            for (int j = 0; j < innerLoop; ++j) {
                ...
                log.debug("Step 6: " + (end-start) + "ms");

                // Update W with Wprime.
                W = Wprime;
            }

            log.debug("Finishedo processing outer loop: " + i);
        }
    }
