...
public class RSS090Generator extends BaseWireFeedGenerator {
	...
	private static final List<String> cdataFields = Arrays.asList("title",
			"description");
	...
	protected void addItem(Item item, Element parent, int index)
		...
		for (String field : cdataFields) {
			Element ele = eItem.getChild(field);
			if (null != ele) {
				String text = ele.getValue();
				ele.setContent(new CDATA(text));
			}
		}
